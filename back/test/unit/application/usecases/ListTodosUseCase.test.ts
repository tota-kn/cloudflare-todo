...
describe('ListTodosUseCase', () => {
  ...
  describe('execute()', () => {
    ...
    it('複数のTodoを含むリストを返す', async () => {
      const todo1 = TestFactory.createTodo({ title: 'タスク1', description: '説明1' })
      const todo2 = TestFactory.createTodo({ title: 'タスク2', description: '説明2' })
    ...
    it('大量のTodoを正しく処理する', async () => {
      ...
      expect(result.every(todo => todo.description?.startsWith('説明'))).toBe(true)
    })
  })
})
